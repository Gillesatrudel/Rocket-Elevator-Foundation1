  ...
  def new_interventions
        ...
        if interventions.save!
          ZendeskAPI::Ticket.create!($client, :type => "task", :priority => "normal",
            ...
            :comment => { :value => "#{interventions.customer.business_name}  customer of building:  #{interventions.building.building_name} in battery:  #{interventions.battery_id} and column:  #{interventions.column_id || "none"} and elevator:  #{interventions.elevator_id || "none"}\n with technician:  #{interventions.employee.name || "none"} report #{interventions.report} by author #{interventions.author_id}"}, 
